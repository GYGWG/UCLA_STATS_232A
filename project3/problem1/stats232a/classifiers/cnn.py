...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        FC_out, FC_cache = fc_relu_forward(conv_out, W2, b2)
        scores, scores_cache = fc_forward(FC_out, W3, b3)
        ...
        ############################################################################
        loss, dydz = softmax_loss(scores, y)
        loss += 0.5 * self.reg * (np.linalg.norm(W1) + np.linalg.norm(W2) + np.linalg.norm(W3))

        dydz, grads['W3'], grads['b3'] = fc_backward(dydz, scores_cache)
        dydz, grads['W2'], grads['b2'] = fc_relu_backward(dydz, FC_cache)
        dydx, grads['W1'], grads['b1'] = conv_relu_pool_backward(dydz, conv_cache)
